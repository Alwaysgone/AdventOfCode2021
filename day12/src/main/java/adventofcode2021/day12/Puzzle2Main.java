...
public class Puzzle2Main {
	...
	public static void main(String[] args) throws IOException {
		try(BufferedReader reader = new BufferedReader(new FileReader(Paths.get(".").toAbsolutePath().resolve("puzzle12_input.txt").toFile()))) {
			...
			.flatMap(sc -> {
				...
				return getPaths(sc, "start," + sc, connectedCaves, startCaves, endCaves, visitedCaves, new HashSet<>(), false).stream();
			})
	...
	private static Set<String> getPaths(String currentCave, String currentPath, Map<String,
		...
		for(String cave : caves) {
			if(!visitedCaves.getOrDefault(cave, false)
					|| !visitedTwice
					|| Character.isUpperCase(cave.toCharArray()[0])) {
				Map<String, Boolean> newVisitedCaves = new HashMap<>(visitedCaves);
				boolean secondVisit = visitedTwice || visitedCaves.containsKey(cave) && Character.isLowerCase(cave.toCharArray()[0]);
				newVisitedCaves.put(cave, true);
				Set<String> subPaths = getPaths(cave, currentPath + "," + cave, connectedCaves, startCaves, endCaves, newVisitedCaves, paths, secondVisit);
