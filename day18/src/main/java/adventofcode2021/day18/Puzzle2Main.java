...
public class Puzzle2Main {
	...
	public static void main(String[] args) throws IOException {
		List<String> snailFishNumbers = new ArrayList<>();
		try(BufferedReader reader = new BufferedReader(new FileReader(Paths.get(".").toAbsolutePath().resolve("puzzle18_input.txt").toFile()))) {
			...
			while((snailFishNumber = reader.readLine()) != null) {
				snailFishNumbers.add(snailFishNumber);
			}
		}
		long largestMagnitude = 0;
		for(int i = 0; i < snailFishNumbers.size(); i++) {
			String number1 = snailFishNumbers.get(i);
			for(int j = 0; j < snailFishNumbers.size(); j++) {
				if(i == j) {
					continue;
				}
				String number2 = snailFishNumbers.get(j);
				String sum = '[' + number1 + ',' + number2 + ']';
				sum = reduceSum(sum);
				long magnitude = calculateMagnitude(sum);
				if(magnitude > largestMagnitude) {
					largestMagnitude = magnitude;
				}
			}
		}
		System.out.println("Largest magnitude is: " + largestMagnitude);
	}
